              ...
              <div key={c.id} className="group rounded-2xl overflow-hidden border border-stone-200 bg-white hover:shadow-md transition">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-stone-600">{c.duration}</span>
                  </div>
                  <h3 className="mt-2 font-semibold text-stone-900 group-hover:text-primary-700">
                    <Link to={`/courses/${c.id}/intro`}>{c.title}</Link>
                  </h3>
                  <div className="mt-4 flex items-center justify-between">
                    {isPro ? (
                      <div className="text-sm font-medium text-stone-900">{formatVND(c.price)}</div>
                    ) : (
                      <div className="text-sm text-green-700 font-medium">Miễn phí</div>
                    )}
                    <Link
                      className={isPro ? "btn btn-primary" : "btn border-stone-300 hover:border-stone-400"}
                      to={`/courses/${c.id}/intro`}
                    >
                      Xem giới thiệu
                    </Link>
                  </div>
