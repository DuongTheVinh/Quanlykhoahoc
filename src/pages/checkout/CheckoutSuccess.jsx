...
export default function CheckoutSuccess() {
  ...
  const order = state || {};
  const title = order?.course ? order.course.title : `Gói ${order?.plan?.name || "Pro"}`;
  const amount = order?.pricing?.total || order?.course?.price || order?.plan?.price || 0;
  return (
    <div className="bg-white">
      <div className="max-w-2xl mx-auto px-4 py-12 text-center">
        ...
        <p className="mt-2 text-stone-600">Cảm ơn bạn đã thanh toán: {title}. Email xác nhận sẽ được gửi tới {order?.info?.email || "email"}.</p>
        ...
        <div className="mt-6 text-left rounded-2xl border border-stone-200 p-6 inline-block text-sm">
          ...
          <div className="flex items-center justify-between mt-2"><span>Sản phẩm</span><span>{title}</span></div>
          <div className="flex items-center justify-between mt-2"><span>Tổng thanh toán</span><span className="font-semibold">{formatVND(amount)}</span></div>
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          {order?.course ? (
            <Link to={`/learn/${order.course.id}`} className="btn btn-primary">Bắt đầu học</Link>
          ) : (
